...
function getPolynom(...args) {
  switch (args.length) {
    ...
    case 3:
      return (x) => x ** 2 * args[0] + x * args[1] + args[2];
    default:
      return null;
  }
}
...
function logger(func, logFunc) {
  return function f(...args) {
    const argsStr = args.map((item) => {
      if (Array.isArray(item)) {
        return `[${item.map((it) => (typeof it === 'string' ? `"${it}"` : it)).join(',')}]`;
      }
 ...
 *   getId10() => 11
 */
function getIdGeneratorFunction(s) {
  let res = s - 1;
  return () => {
    res += 1;
    return res;
