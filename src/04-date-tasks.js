...
function timeSpanToString(s, e) {
  const time = new Date(e.getTime() - s.getTime());
...
function angleBetweenClockHands(date) {
  const h = date.getUTCHours() > 11 ? date.getUTCHours() - 12 : date.getUTCHours();
  const m = date.getMinutes();
  const hDeg = (h * 60 + m) * 0.5;
  const mDeg = m * 6;
  const deg = Math.abs(hDeg - mDeg) > 180 ? Math.abs(hDeg - mDeg) - 180 : Math.abs(hDeg - mDeg);
